...
| **어려움을 결정하는 것은 로봇 베이스까지의 거리다** | 거리만 바꾸면 0.58 m 에서 19/20 (95 %), 0.78 m 에서 0/20. Pearson −0.956, p = 0.0007. 제어 오차를 빼도 상관이 +0.922 로 남는다 |
| **학습 위치에서 멀어져서가 아니다** | 학습 위치에 가장 가까운 조건이 0/20, 가장 먼 조건이 19/20. 성공과 학습 배치 거리의 상관은 +0.512 (p = 0.24) 로 유의하지 않다 |

네 번째부터 여섯 번째까지가 이 저장소의 주된 결과다.
기하학적 난이도와 학습 배치 거리를 난이도 축으로 잡은 두 실험은 기각됐고,
세 번째로 세운 작업공간 거리 가설이 확인됐다.
...
요약본: [results/보고_요약.md](results/보고_요약.md)
...
- **작업공간 거리 가설은 사후에 발견해 별도 실험으로 확인했다.** 다만 그 실험에서
  거리와 학습 위치 대비 거리가 −0.958로 겹친다. 완전한 분리는 못 했다.
...
**하락은 확실하다.** 다만 처음 세운 두 난이도 축은 성공을 예측하지 못했다.
답은 7절의 세 번째 축에 있다.

![30세트 배치](assets/setup-map.png)

세트별 개별 배치는 [setup-grid.png](assets/setup-grid.png)에 있다.
...
500판의 태스크 수준에서도 재현된다 (Pearson +0.650, p = 0.042).
...
**거리만 변하고 물체 간 거리·순서·방위 관계는 고정된다.** 수준당 20판, 총 140판.

![거리와 성공률](assets/reach-success.png)

| 거리 | 정렬 오차 중앙 | 들기 | 도달 | 배치 |
|---|---|---|---|---|
| 0.580 | 28.4 mm | 20/20 | 19/20 | **19/20 (95 %)** |
| 0.615 | 22.7 mm | 19/20 | 15/20 | **15/20 (75 %)** |
| 0.650 | 26.7 mm | 14/20 | 7/20 | **7/20 (35 %)** |
| 0.685 | 38.5 mm | 20/20 | 7/20 | **7/20 (35 %)** |
| 0.720 | 42.2 mm | 20/20 | 2/20 | **1/20 (5 %)** |
| 0.750 | 49.2 mm | 20/20 | 1/20 | **1/20 (5 %)** |
| 0.780 | 59.8 mm | 20/20 | 0/20 | **0/20 (0 %)** |
...
정렬 오차 변화 31.5 mm 중 제어 오차 변화는 5.4 mm (17 %)
```
...
확인 실험에서 그릇을 앞으로 당기면 자동으로 학습 위치(거리 0.793 m)에서 멀어진다.
...
**학습 위치에 가장 가까운 조건이 전멸하고 가장 먼 조건이 95 %다.**
"학습 위치에서 멀어져서 못한다"는 설명은 배제된다.

다만 확인 실험의 설계상 거리와 학습 위치 거리가 −0.958로 겹친다.
완전한 분리는 같은 거리에서 학습 위치 대비 거리만 바꾸는 조건이 필요하다.

### 이전 결과들이 설명된다

배치 무작위화 실험 30세트의 거리 범위는 0.658 ~ 0.899 m다. 확인 실험에서 그 구간은
성공률이 35 % 이하로 무너지는 영역이다. **17 %라는 결과가 여기서 나온다.**
...
- [ ] 같은 거리에서 학습 위치 대비 거리만 바꾸는 조건 (두 축 완전 분리)
- [ ] 다른 태스크에서도 같은 거리 곡선인지 확인
